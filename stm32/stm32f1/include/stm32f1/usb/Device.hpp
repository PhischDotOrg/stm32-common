...
public:
    /*
     * Helper Class to represent the USB FIFO Memory Elements.
     */
    class FifoMemory {
    public:
    ...
    private:
        ...
        uint8_t m_padding[2];
    #else
        uint8_t m_padding[2] __attribute__((unused));
    #endif

    public:
        FifoMemory() = default;
        constexpr FifoMemory(uint16_t p_data) : m_data(p_data), m_padding {} { };

        operator int() const { return m_data; };
    };
    static_assert(sizeof(FifoMemory) == 4);
    static_assert(sizeof(FifoMemory::m_data) == 2);

    /*
     * Array contains Elements of type "FifoMemory" which are 4 Bytes long. They contain two
     * Bytes of payload and two bytes of padding.
     *
     * Therefore, an array w/ 32 Elements will be able to store 64 Bytes, but use 128 Bytes of
     * address space.
    */
    template<unsigned t_Length>
    class FifoSpace {
        std::array<FifoMemory, t_Length / 2> m_data;
    public:
        constexpr size_t size() const noexcept {
            return m_data.size() * sizeof(FifoMemory::m_data);
        }

        constexpr FifoMemory * data() noexcept {
            return m_data.data();
        }

        constexpr const FifoMemory * data() const noexcept {
            return m_data.data();
        }
    };
    ...
    void setAddress(uint8_t p_address, const bool p_statusStageComplete = true) const override;
...
public:
     ...
     */
    virtual ~DeviceT() {
        m_nvic.disableIrq(* static_cast<EngineT<Address> *>(this));
